...
def _bounded_points(rng: np.random.Generator, n: int) -> tuple[np.ndarray, np.ndarray]:
    """Create synthetic points around a realistic Colorado-like bounding box."""
    lat_min, lat_max = 37.2, 40.9
    lon_min, lon_max = -109.0, -102.2
    ...
    weights = np.array([0.48, 0.18, 0.14, 0.10, 0.10])
    cluster = rng.choice(len(centers), size=n, p=weights)
    lat = rng.normal(centers[cluster, 0], rng.uniform(0.12, 0.55, n))
    lon = rng.normal(centers[cluster, 1], rng.uniform(0.14, 0.75, n))
...
def build_synthetic_records(row_count: int = ROW_COUNT, seed: int = RANDOM_SEED) -> pd.DataFrame:
    ...
    distances = np.sqrt(((latitude[:, None] - urban_centers[:, 0]) * 69) ** 2 + ((longitude[:, None] - urban_centers[:, 1]) * 53) ** 2)
    ...
    base_class += 0.45 * np.sin((longitude + 106.2) * 2.4) + 0.35 * np.cos((latitude - 38.4) * 3.1)

    pockets = [
        (39.15, -105.35, -1.4, 0.28),
        (38.25, -106.05, 1.2, 0.36),
        (40.25, -103.75, 1.4, 0.42),
        (37.95, -108.35, 1.0, 0.32),
    ]
    ...
    return pd.DataFrame(
        {
            ...
            "region": "Synthetic Mountain Region",
